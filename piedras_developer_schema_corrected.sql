...
-- Safe schema setup for a fresh Supabase project.
...
-- 4. Constraints
-- Primary keys are created first so later foreign keys can reference them safely.
...
-- Foreign keys are created after primary/unique keys exist.
...
begin
  ...
  -- Use the locked values from the purchase link. The link is what the developer sent to the buyer.
  v_payment_plan_mode_text := coalesce(
    nullif(trim(coalesce(v_link.payment_plan_mode, '')), ''),
  ...
  if v_payment_plan_mode_text not in ('outright', 'fixed_installment', 'flexible') then
    raise exception 'Purchase link payment plan mode is invalid.';
  end if;
  ...
  v_total_price := round(coalesce(v_link.total_price, v_plot.price, 0), 2);
  v_first_payment := round(coalesce(v_link.first_payment_amount, 0), 2);
  ...
  if v_payment_plan_mode = 'outright'::public.developer_payment_plan_mode
    and round(v_first_payment, 2) <> round(v_total_price, 2) then
    raise exception 'Outright payment requires first payment to equal total price.';
  end if;

  v_balance := round(v_total_price - v_first_payment, 2);

  if v_balance > 0 and (v_balance_spread_months < 1 or v_balance_spread_months > 120) then
